...
var ArrAll = {
    lessDir : 'src/css',      //需要解析的less文件目录
    cssDir : 'src/css',       //需要解析的less文件目录
    cssRes : 'build/css/res', //生成的css目录
    cssMin : 'build/css' ,    //生成的min.css目录
    jsSrc : 'src/js',         //js目录
    jsRes : 'build/js/res',   //js目录
    jsMin : 'build/js',       //生成的min.js目录
    imgSrc : 'src/images/',   //原图片路径
    imgMin: 'build/images/',  //图片生成路径 
...
gulp.task('testLess', function () {
    gulp.src([ArrAll.lessDir+'/*.less','!'+ArrAll.lessDir+'/_*.less']) //该任务针对的文件和不对其进行处理的文件
        ...
        .pipe(gulp.dest(ArrAll.cssRes))     //生成css文件
...
gulp.task('watch',function(){
  ...
  gulp.watch(ArrAll.cssDir+'/_*.css',['cssConcat']);   //合并css文件，并进行优化处理
	gulp.watch(ArrAll.jsSrc+'/_*.js',['jsConcat']);
